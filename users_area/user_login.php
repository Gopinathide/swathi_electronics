...
<?php
        if(isset($_POST['user_register']))
        {
            ...
            $select_query="Select * from `user_table` where user_name='$user_name' and verify=1";
            $result=mysqli_query($con,$select_query);
            $row_count=mysqli_num_rows($result);
            $row=mysqli_fetch_assoc($result);
            $ip=getIPAddress();
            $sel="Select * from `user_table` where user_name='$user_name' and user_password='$user_password'";
            $sell=mysqli_query($con,$sel);
            $sells=mysqli_num_rows($sell);
            if($sells==1)
            {
            ...
            {
                echo "<script>alert('$row_count Invalid Crediantls')</script>";
            }
        }
        ?>